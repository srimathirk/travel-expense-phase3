...
def main():
    ...
    clear_screen(1)
    user_name = input(blue("Enter user name, checking if user exists already: "))  #adding user CRUD
    if user_exists(user_name):
        print(green(f"User already exists!!"))
        displaying_user(user_name)
        clear_screen(1)
    else:
        print(red("User do not exist. add user details"))
    ...
    print(yellow(f"You selected {user_options[user_entry_index]}!"))
    selected_user_option = user_options[user_entry_index]
    if selected_user_option == "update":
        while True:
            update = input(yellow("Do you want to update User details? (yes/no)")).lower()
            if update == 'yes':
        ...
        while True:
            delete = input(yellow("Do you want to delete User details? (yes/no)")).lower()
            if delete == 'yes':
    ...
    print(blue(f"Checking if trip details exists already: "))
    if trip_exists(user_name):
        print(f"Trips already exists with {user_name} in database !!")
        clear_screen(1)
        displaying_trips(user_name)
        clear_screen(1)
        add = input(yellow("Do you want to add more trips? (yes/no)")).lower()
        if add == 'yes':
        ...
        print(red("No trips exist with this user. add new trip details"))
        clear_screen(1)
        start_place,end_place,avg_gas_price,fuel_efficiency_mpg = get_trip()
        user_id = get_user_id(user_name)
        saving_tripdetails_db(start_place,end_place,avg_gas_price,fuel_efficiency_mpg,user_id)
    ...
    print(yellow(f"You selected {trip_options[trip_entry_index]}!"))
    selected_trip_option = trip_options[trip_entry_index]
    if selected_trip_option == "update":
        while True:
            update = input(yellow("Do you want to update Trip details? (yes/no)")).lower()
        ...
        while True:
            delete = input(yellow("Do you want to delete Trip details? (yes/no)")).lower()
            if delete == 'yes':
    ...
    print(blue(f"Checking if expense details exists already: "))
    if expense_exists(user_name):
        ...
        print(red("no expense details found for this user"))
    ...
    print(yellow(f"You selected {expense_options[expense_entry_index]}!"))
    selected_expense_option = expense_options[expense_entry_index]
    if selected_expense_option == "update":
        while True:
            update = input(yellow("Do you want to update Expense details? (yes/no)")).lower()
        ...
        while True:
            delete = input(yellow("Do you want to delete Expense details? (yes/no)")).lower()
            if delete == 'yes':
                expense_id = get_expense_id(user_name)
        ...
        print(blue(f"Calculating expense details for {user_name}"))
    time.sleep(1)
    ...
    print(green("Calculating total expenses cost for entire trip"))
    clear_screen(1)
    calculate_total_expenses(user_name)
    clear_screen(1)
    print(yellow("Thank you for using cli travel app"))
